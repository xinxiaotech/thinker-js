...
class Thinker extends EventEmitter {
  ...
  _intoSync() {
    const autoSwitchStatus = () => {
      this._autoSwitchStatusTimer = setTimeout(() => {
        this._autoSwitchStatusTimer = null
  ...
  _callSync(time = 1) {
    ...
    const option = this._mergePassInOptions(passInOptions)
  ...
  _mergePassInOptions(passInOptions) {
    const anySyncAllData = _some(passInOptions, 'syncAllData')
  ...
  _doSync(syncInfo, option) {
    ...
    return this.initOption.call(getDataMethodName, syncInfo).then((data) => {
      ...
      if (
        ...
        !this._backgroundInitializeCompletelySyncing &&
        ...
        this._backgroundInitializeCompletelySyncing = true
        this._doSync(completelySyncInfo, option).then(() => {
          this._backgroundInitializeCompletelySyncing = false
  ...
  _updateLocalData(syncInfo, resp) {
    ...
    return this.initOption.call('getItems', syncInfo, itemIds).then((localData) => {
      const groupedPromises = _mapValues(localData, (localItems, type) => {
        ...
        if (type === 'todo_order') {
          ...
          const deserializingActionPromiseGroups = _reduce(itemIds[type], (memo, itemId) => {
            ...
            memo[action][itemId] = this.initOption.call('deserializeItem', syncInfo, type, action, requestItem, localItem, serverItem)
            return memo
          }, {})
      ...
      return this.initOption.call('updateLocalData', syncInfo, groupedData).then(() => resp)
    })
  }
}
