  ...
  _doSync(syncInfo, option) {
    ...
    return this.initOption[getDataMethodName](syncInfo).then((data) => {
      ...
      if (this.initOption.autoRetryRequestCount) {
        ...
        return utils.delayRetry((retryTime) => {
      ...
      const groupedData = _mapValues(localData, (localItems, type) => {
        ...
        if (type === 'todo_order') {
          ...
          if (requestItems != null) {
            requestItemsMappedLocalItems = _filter(localItems, (localItem, id) => requestItems[id] != null)
          } else {
            requestItemsMappedLocalItems = []
          }
          return {keepDirty: requestItemsMappedLocalItems}
        } else {
          return _reduce(itemIds[type], (memo, itemId) => {
