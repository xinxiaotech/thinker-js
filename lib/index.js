...
class Thinker extends EventEmitter {
  ...
  constructor(passInOption) {
    ...
    Promise.resolve(this.initOption.storage.getItem(CONSTS.INIT_COMPLETELY_SYNC_STATUS_STORAGE_KEY)).then((backgroundInitializeCompletelySyncSucceed) => {
      // 如果后台完整同步在之前就已经完成了，那么告知调用者也是越早越好
      if (backgroundInitializeCompletelySyncSucceed == true) {
        /**
         * 在后台初次完整同步后触发
         *
  ...
  _callSync(time = 1) {
    return Promise.resolve(this.initOption.storage.getItem(CONSTS.LAST_SYNC_STORAGE_KEY)).then((lastSuccessTime) => {
      const reasons = _map(this._currentSync, 'reason')
      const passInOptions = _map(this._currentSync, 'passInOption')
      const syncInfo = { passInOptions, isFirstTime: lastSuccessTime == null }
  ...
  _tryToTriggerBackgroundInitializeCompletelySync(syncInfo, passInOption) {
    if (!this.initOption.autoBackgroundCompletelySync) return
    if (this.backgroundSyncing || syncInfo.background) return
    return Promise.resolve(this.initOption.storage.getItem(CONSTS.INIT_COMPLETELY_SYNC_STATUS_STORAGE_KEY)).then((backgroundInitializeCompletelySyncSucceed) => {
      if (backgroundInitializeCompletelySyncSucceed) return
      ...
      return this._doSync(completelySyncInfo, option).then(() => {
    ...
    return Promise.resolve(this.initOption.storage.getItem(CONSTS.LAST_SYNC_STORAGE_KEY)).then((lastSuccessTime) => {
      ...
      if (option.syncAllData) {
        ...
        if (!lastSuccessTime) {
          ...
          requestData = {last_sync: utils.parseValidDate(lastSuccessTime).toJSON(), objects: data}
        }
      }
  ...
  _updateLocalData(syncInfo, resp) {
    /*
        typeof serverData and requestData:
        { [entityType: string]: { [entityId: string]: Entity } }

        see pickObjectsIds and pickObjectsIds
    */
    const requestData = resp.config.data.objects
    const serverData = resp.data.objects

    const itemIds = {} // type: { [entityType: string]: string[] }
    ...
    return this.initOption.call('getItems', syncInfo, itemIds).then(localData =>
      utils.props(_mapValues(itemIds, (ids, type) => {
        // type: 'timer' | 'task'
        const requestItems = requestData[type] // { [entityId: string]: Entity } }
        const localItems = localData[type]
        const serverItems = serverData[type] // { [entityId: string]: Entity } }
        return this._deserializeItem(syncInfo, type, ids, requestItems, localItems, serverItems, resp)
      })) // type of props: (p1: { [entityType: string]: Promise<> })
    ).then(groupedData => {
  ...
  _deserializeItem(syncInfo, type, itemIds, requestItems, localItems, serverItems, resp) {
    ...
    if (type === 'todo_order') {
      ...
      // itemIds: { [entityType: string]: string[] }
