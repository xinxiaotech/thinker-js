 ...
 * @property {Boolean} background - 本轮同步的最后一次触发是否是 `backgroundInitializeCompletelySync`
 * @property {Thinker~SyncOption[]} passInOptions - 被合并到本轮同步里的调用 `do()` 时传入所有设置的数组
 */

/**
 * @typedef {Object} ThinkerType~SyncOption
 * @property {?Number} debounce - 在本次同步内覆盖 `{@link Thinker.defaultOption.debounce}`
 * @property {?Number} timeout - 在本次同步内覆盖 `{@link Thinker.defaultOption.timeout}` 或 `{@link Thinker.defaultOption.syncAllDataTimeout}`，如果在多次 `Thinker#do` 调用里传入了不同的 `timeout` ，则使用最小的数值
...
/**
 * @typedef {String} ThinkerType~ObjectAction - 客户端需要对本地数据进行的若干种操作，目前有四种：keepDirty, cleanDirty, update, new
 */
